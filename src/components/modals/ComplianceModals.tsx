...
type ExclusionLength = '24h' | '7d' | '30d' | '6m' | 'permanent';
const EXCLUSION_OPTIONS: { id: ExclusionLength; label: string; sub: string; permanent?: boolean }[] = [
  { id: '24h',       label: '24 hours',  sub: 'Quick break · ends automatically' },
  { id: '7d',        label: '7 days',    sub: 'Recommended if you need space' },
  { id: '30d',       label: '30 days',   sub: 'Longer cool-off' },
  { id: '6m',        label: '6 months',  sub: 'Extended exclusion · requires support to reverse' },
  { id: 'permanent', label: 'Permanent', sub: 'Cannot be reversed without contacting support', permanent: true },
];
...
export function SelfExclusionModal({
  ...
  const isPermanent = selected === 'permanent';
  const canConfirm = step === 'confirm' && selected && (!isPermanent || confirmText.trim().toUpperCase() === 'EXCLUDE');
      ...
      {step === 'choose' && (
        <div className="p-5 space-y-3">
          <p className="text-[12px] leading-relaxed" style={{ color: '#8FA3B8' }}>
            Choose a period to pause your account. During exclusion you can&apos;t play, deposit, or withdraw. Permanent exclusion is irreversible without contacting support.
          </p>
            ...
            {EXCLUSION_OPTIONS.map((o) => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: isSel ? (o.permanent ? 'rgba(239,68,68,0.10)' : 'rgba(240,178,50,0.10)') : 'rgba(255,255,255,0.02)',
                    border: `1px solid ${isSel ? (o.permanent ? 'rgba(239,68,68,0.45)' : 'rgba(240,178,50,0.45)') : '#1A2238'}`,
                  }}
                >
                  <div
                    className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5"
                    style={{
                      background: o.permanent ? 'rgba(239,68,68,0.14)' : 'rgba(240,178,50,0.10)',
                      border:     `1px solid ${o.permanent ? 'rgba(239,68,68,0.30)' : 'rgba(240,178,50,0.30)'}`,
                      color:      o.permanent ? '#EF4444' : '#F0B232',
                    }}
                  >
                    {o.permanent ? <Ban className="w-4 h-4" /> : <Clock className="w-4 h-4" />}
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-bold" style={{ color: o.permanent ? '#EF4444' : '#F5E8C8' }}>{o.label}</p>
                    <p className="text-[11px] mt-0.5" style={{ color: '#8FA3B8' }}>{o.sub}</p>
                  </div>
                  {isSel && (
                    <Check className="w-4 h-4 flex-shrink-0 mt-1.5" style={{ color: o.permanent ? '#EF4444' : '#F0B232' }} />
                  )}
                </button>
              );
            })}
          ...
          >
            ...
            <p className="text-[11px] leading-relaxed" style={{ color: '#8FA3B8' }}>
              ...
              <a href="tel:1-800-522-4700" className="font-bold" style={{ color: '#60A5FA' }}>1-800-522-4700</a> (24/7 · free · confidential).
            </p>
          </div>
      ...
      {step === 'confirm' && selected && (
        <div className="p-5 space-y-3">
          <div
            ...
            style={{
              background: isPermanent ? 'rgba(239,68,68,0.10)' : 'rgba(240,178,50,0.10)',
              border: `1px solid ${isPermanent ? 'rgba(239,68,68,0.40)' : 'rgba(240,178,50,0.40)'}`,
            }}
          >
            <AlertTriangle className="w-5 h-5 flex-shrink-0 mt-0.5" style={{ color: isPermanent ? '#EF4444' : '#F0B232' }} />
            <div>
              <p className="text-sm font-bold" style={{ color: isPermanent ? '#EF4444' : '#F0B232' }}>
                {isPermanent ? 'This is permanent' : `You're excluding yourself for ${EXCLUSION_OPTIONS.find((o) => o.id === selected)?.label}`}
              </p>
              <p className="text-[11px] mt-1 leading-relaxed" style={{ color: '#F5E8C8' }}>
                {isPermanent
                  ? 'Your account will be permanently closed. You cannot reverse this without contacting support, and approval is not guaranteed.'
            ...
            <div>
              ...
              <input
                ...
                style={{
                  ...
                  border: `1px solid ${confirmText.trim().toUpperCase() === 'EXCLUDE' ? 'rgba(45,201,122,0.45)' : '#1A2238'}`,
                  color: '#F5E8C8',
                }}
              />
            </div>
            ...
            <button
              ...
              style={{
                background: isPermanent
                  ? 'linear-gradient(135deg, #EF4444, #DC2626)'
                  : 'linear-gradient(135deg, #F0B232, #FFD166)',
                color: isPermanent ? '#fff' : '#040814',
                boxShadow: isPermanent ? '0 4px 16px rgba(239,68,68,0.30)' : '0 4px 16px rgba(240,178,50,0.30)',
              }}
            >
              {isPermanent ? 'Permanently exclude me' : 'Confirm exclusion'}
            </button>
          </div>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="p-5 space-y-3 max-h-[78vh] overflow-y-auto no-scrollbar">
        ...
        <Field label="Federal tax classification">
          <select
            ...
            style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid #1A2238', color: '#F5E8C8' }}
          >
            <option value="individual"  style={{ background: '#0F1828' }}>Individual / sole proprietor</option>
            <option value="sole-prop"   style={{ background: '#0F1828' }}>Single-member LLC</option>
            <option value="llc"         style={{ background: '#0F1828' }}>LLC (taxed as partnership / corporation)</option>
            <option value="corp"        style={{ background: '#0F1828' }}>C / S corporation</option>
            <option value="partnership" style={{ background: '#0F1828' }}>Partnership</option>
            <option value="other"       style={{ background: '#0F1828' }}>Other</option>
          </select>
        </Field>
        ...
        <div className="grid grid-cols-[110px_1fr] gap-2">
          <Field label="TIN type">
            <select
              ...
              style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid #1A2238', color: '#F5E8C8' }}
            >
              <option value="ssn" style={{ background: '#0F1828' }}>SSN</option>
              <option value="ein" style={{ background: '#0F1828' }}>EIN</option>
            </select>
          </Field>
