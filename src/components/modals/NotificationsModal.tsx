...
function IconBadge({ kind }: { kind: NotifKind }) {
  switch (kind) {
    ...
    case 'system':  return <Bell      className="w-4 h-4 text-[#8FA3B8]" />;
  }
}
      ...
      {notificationsOpen && (
        <>
          ...
          >
            ...
            <div className="p-3" style={{ borderBottom: '1px solid #1A2238', background: 'rgba(45,201,122,0.04)' }}>
              <div className="flex items-center gap-3 rounded-xl p-3"
                style={{
                  background: rakebackBalance > 0
                    ? 'linear-gradient(135deg, rgba(45,201,122,0.10), rgba(45,201,122,0.04))'
                    : 'rgba(255,255,255,0.03)',
                  border: `1px solid ${rakebackBalance > 0 ? 'rgba(45,201,122,0.28)' : '#1A2238'}`,
                }}
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs number-display" style={{ color: '#8FA3B8' }}>
            ...
            <div className="max-h-[420px] overflow-y-auto no-scrollbar">
              {preview.length === 0 ? (
                <div className="px-4 py-8 text-center">
                  <Bell className="w-6 h-6 mx-auto mb-2" style={{ color: '#4A5878' }} />
                  <p className="text-xs" style={{ color: '#8FA3B8' }}>You&apos;re all caught up.</p>
                </div>
                ...
                return (
                  <button
                    ...
                    style={{ borderBottom: '1px solid rgba(26,34,56,0.55)' }}
                  >
                    <div
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-[11px] mt-0.5 leading-snug truncate" style={{ color: '#8FA3B8' }}>{n.body}</p>
                      <p className="text-[10px] mt-1 font-mono" style={{ color: '#4A5878' }}>{relTime(n.ts)}</p>
                    </div>
                  </button>
                );
              })}
            ...
            {/* Footer */}
            <div className="px-4 py-2.5 flex items-center justify-between gap-3" style={{ background: '#08121C', borderTop: '1px solid #1A2238' }}>
              <div className="flex items-center gap-2">
                {unreadTotal > 0 && (
                  <button
